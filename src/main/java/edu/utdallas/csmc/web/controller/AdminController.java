...
public class AdminController {
    ...
    @RequestMapping("/home/lookUp")
    public String adminHomePageUpdate(ModelMap model, @RequestParam("date") @DateTimeFormat(pattern = "yyyy-MM-dd") Date date) {
        model = adminHomeService.getHomeScreenDetails(model, date);
    ...
    @RequestMapping(
            value = {"/session/edit/uploadFile/{sessionId}/{sessionType}" },
    ...
    public String uploadFile(
        ...
        for (MultipartFile file : files) {
            if (file != null && !file.isEmpty()) {
                System.out.print("file upload: " + file.getOriginalFilename());
                try {
                    ...
                    String uploadLocation = "./src/main/resources/uploads/" + directoryLevelOne + "/" + directoryLevelTwo + "/" + path;
                    ...
                    File directory = receivedFile.getParentFile();
                    ...
                    if (!receivedFile.exists()) {
                        // Save file to disk
                        try (OutputStream outStream = new FileOutputStream(receivedFile)) {
                            outStream.write(buffer);
                            log.info("Saved uploaded file to: " + uploadLocation);
                        ...
                        log.info("File already exists: " + uploadLocation);
                    }
                } catch (Exception e) {
                    log.error("Error uploading file: " + file.getOriginalFilename(), e);
                }
            } else {
                log.warn("Empty or null file encountered, skipping...");
            }
        }
    ...
    public String generateReportTypeForm(ModelMap model) {
        return "role/admin/report/report_main.html";
    }

    @RequestMapping(path = "/report/sectionSession")
    ...
    @RequestMapping(path = "/report/ajax/courseVisit/semester/{semester}", method = RequestMethod.POST)
    public String ajaxSelectedSemesterAction(@PathVariable String semester, ModelMap model) {
        log.info("Front end to back end passing data ===> "+semester);
        return "redirect:/admin/report/semester/{semester}";
    }

    @RequestMapping(path = "/report/semester/{semester}", method = RequestMethod.GET)
    public String semesterDependency( @PathVariable String semester, ModelMap model) throws ParseException {
        log.info("Entered Semester dependency method!!");
        ...
        String formattedStartDate = outputDateFormat.format(startDate_date);
        String formattedEndDate = outputDateFormat.format(endDate_date);

        Map<String, Integer> courseVisits = adminCourseService.getCourseVisitsDetailsForDateRange(formattedStartDate ,formattedEndDate);

        model.addAttribute("courseVisits", courseVisits);
        model.addAttribute("semester",semester);
        model.addAttribute("startDate",formattedStartDate);
        model.addAttribute("endDate", formattedEndDate);
        return "role/admin/report/report_semester.html";
    }

    @RequestMapping(path = "/report/courseVisitSemester")
    public String generateCourseVisitsEntireSemesterReport(ModelMap model) {
        List<String> semesters = new ArrayList<>();
        List<AdminSemeterResultDTO> semesterList = adminSemesterService.getSemesterList();
        for(AdminSemeterResultDTO adminSemeterResultDTO : semesterList) {
            semesters.add(adminSemeterResultDTO.getSeason() + " " + adminSemeterResultDTO.getYear());
        }
        model.addAttribute("semesters", semesters);
        return "role/admin/report/report_courseVisit_semester.html";
    }

    @RequestMapping(path = "/report/ajax/mentor/semester/{semester}", method = RequestMethod.POST)
    public String ajaxSelectedSemesterMentorHoursAction(@PathVariable String semester, ModelMap model) {
        log.info("Front end to back end passing data ===> "+semester);
        return "redirect:/admin/report/mentor/semester/{semester}";
    }

    @RequestMapping(path = "/report/mentor/semester/{semester}", method = RequestMethod.GET)
    public String semesterDependencyMentorHours( @PathVariable String semester, ModelMap model) throws ParseException {
        log.info("Entered Semester dependency method!!");
        log.info("semester ===> "+semester);
        String[] sem = semester.split(" ");
        String season = sem[0];
        ...
        Map<String, Float> mentorHours = adminMentorService.getMentorHoursForSelectedSemester(startDate_date ,endDate_date);

        mentorHours.forEach((key, value) -> System.out.println("Key: " + key + ", Value: " + value));

        model.addAttribute("mentorHours", mentorHours);
        model.addAttribute("semester",semester);
        model.addAttribute("startDate",startDate_date);
        model.addAttribute("endDate", endDate_date);
        return "role/admin/report/report_mentors_hours_semester.html";
    }

    @RequestMapping(path = "/report/mentorHoursSemester")
    public String generateMentorHoursEntireSemesterReport(ModelMap model) {
        List<String> semesters = new ArrayList<>();
        List<AdminSemeterResultDTO> semesterList = adminSemesterService.getSemesterList();
        for(AdminSemeterResultDTO adminSemeterResultDTO : semesterList) {
            semesters.add(adminSemeterResultDTO.getSeason() + " " + adminSemeterResultDTO.getYear());
        ...
        model.addAttribute("semesters", semesters);
        return "role/admin/report/report_semester_mentors_hours.html";
    }


    @RequestMapping(path = "/report/ajax/mentor/weekly/{semester}", method = RequestMethod.POST)
    public String ajaxSelectedWeeklyMentorHoursAction(@PathVariable String semester, ModelMap model) {
        log.info("Front end to back end passing data ===> "+semester);
        return "redirect:/admin/report/mentor/weekly/{semester}";
    }

    @RequestMapping(path = "/report/mentor/weekly/{semester}", method = RequestMethod.GET)
    public String weeklyDependencyMentorHours( @PathVariable String semester, ModelMap model) throws ParseException {
        log.info("Entered Semester dependency method!!");
        log.info("semester ===> "+semester);
        String[] sem = semester.split(" ");
        String season = sem[0];
        ...
        Map<Integer, Map<String, String>> weeklyDates = adminSemesterService.getWeeklyDatesGivenSemesterDates(startDate_date ,endDate_date);

        model.addAttribute("weeklyDates", weeklyDates);
        model.addAttribute("semester",semester);
        model.addAttribute("startDate",formattedStartDate);
        model.addAttribute("endDate", formattedEndDate);
        return "role/admin/report/report_mentors_hours_weekly_temp.html";
    }

    @RequestMapping(path = "/report/mentorHoursWeekly")
    public String generateMentorHoursWeeklyReport(ModelMap model) {
        List<String> semesters = new ArrayList<>();
        List<AdminSemeterResultDTO> semesterList = adminSemesterService.getSemesterList();
        for(AdminSemeterResultDTO adminSemeterResultDTO : semesterList) {
            semesters.add(adminSemeterResultDTO.getSeason() + " " + adminSemeterResultDTO.getYear());
        ...
        model.addAttribute("semesters", semesters);
        return "role/admin/report/report_weekly_mentors_hours.html";
    }

    @RequestMapping(path = "/report/mentorTimesheetWeekly/", method = RequestMethod.POST)
    public String getMentorsHoursWeeklyReportByWeek(ModelMap model,
                                                    @ModelAttribute("selectedWeek") String selectedWeekDates) throws ParseException {

        String[] weekDates = selectedWeekDates.split("-");
        String weekStartDate = weekDates[0];
        String weekEndDate = weekDates[1];

        // Define the date-time format for input and output
        SimpleDateFormat inputDateFormat = new SimpleDateFormat("MM/dd/yyyy");

        // Parse the input date-time string
        Date startDate_date = inputDateFormat.parse(weekStartDate);
        Date endDate_date = inputDateFormat.parse(weekEndDate);

        Map<String, Float> mentorWeeklyHours = adminMentorService.getMentorHoursForSelectedSemester(startDate_date ,endDate_date);

        model.addAttribute("mentorWeeklyHours", mentorWeeklyHours);
        model.addAttribute("startDate",weekStartDate);
        model.addAttribute("endDate", weekEndDate);
        return "role/admin/report/report_mentors_hours_weekly.html";
    }

    @RequestMapping(path = "/report/ajax/courseVisit/dateRange/{dateValue}", method = RequestMethod.POST)
    public String ajaxSelectedDateRangeAction(@PathVariable String dateValue, ModelMap model) {
        log.info("Front end to back end passing data ===> "+dateValue);
        return "redirect:/admin/report/dateRange/{dateValue}";
    }

    @RequestMapping(path = "/report/dateRange/{dateValue}", method = RequestMethod.GET)
    public String dateRangeDependency( @PathVariable String dateValue, ModelMap model) {
        log.info("Entered date Range dependency method!!");
        log.info("date Value ===> "+dateValue);
        String[] dateRange = dateValue.split(",");
        ...
        String formattedStartDate = LocalDate.parse(dateRange[0], inputFormatter).format(outputFormatter);
        String formattedEndDate = LocalDate.parse(dateRange[1], inputFormatter).format(outputFormatter);
        ...
        model.addAttribute("startDate",formattedStartDate);
        model.addAttribute("endDate",formattedEndDate);
        return "role/admin/report/report_dateRange.html";
    }

    @RequestMapping(path = "/report/courseVisitDateRange")
    public String generateCourseVisitsDateRangeReport(ModelMap model) {
        List<String> courses = new ArrayList<>();
        List<AdminCoursesResultDTO> supportedAdminCourseResultDTOList = adminCourseService.getSupportedCourseDetails();
        for(AdminCoursesResultDTO adminCoursesResultDTO : supportedAdminCourseResultDTOList) {
            courses.add(adminCoursesResultDTO.getNumber());
        }
        model.addAttribute("courses",courses);
        return "role/admin/report/report_courseVisit_dateRange.html";
    }

    @RequestMapping(path = "/report/allCourseVisitSemester")
    public String generateAllCourseVisitsSemesterReport(ModelMap model) throws ParseException {
        List<String> courses = new ArrayList<>();

        Map<String, Integer> courseVisits =  adminCourseService.getCourseVisitsDetailsForLastFiveSemesters();
        model.addAttribute("courseVisits",courseVisits);
        return "role/admin/report/report_all_courseVisit_semester.html";
    }

    @RequestMapping(path = "/report/courseVisitWeekly")
    public String generateCourseVisitsWeeklyReport(ModelMap model) throws ParseException {
        Map<String, Map<String, Integer>> courseVisits =  adminCourseService.getCourseVisitsDetailsWeeklyByDayOfWeek();
        ObjectMapper objectMapper = new ObjectMapper();
        String json = "{}";
        try {
            json = objectMapper.writeValueAsString(courseVisits);
        } catch (JsonProcessingException e) {
            System.out.println("Error");
        }
        model.addAttribute("courseVisits", json);
        return "role/admin/report/report_courseVisit_weekly.html";
    }
    ...
    public void downloadSessionRequestFiles(HttpServletResponse response, @PathVariable("fileId") String fileId, @PathVariable("requestId") String requestId) {
        ...
        System.out.print("/src/main/resources/uploads/" + directoryLevelOne + "/" + directoryLevelTwo + "/" + path);
        ...
        if (file.exists())
        {
            try
            {
                ...
                System.out.print("file inputStream");
                FileCopyUtils.copy(inputStream, response.getOutputStream());
                response.getOutputStream().flush();
                System.out.print("file flush");
            ...
            System.out.print("error while downloading");
        }
    }

    /*** Admin Ticket board ***/

    @RequestMapping(path = "/ticket/stats")
    public String getTicketStats(ModelMap model) throws ParseException {
        model.addAttribute("mentors", adminMentorService.getActiveMentors());
        return "role/admin/ticket/mentors.html";
    }

    @RequestMapping(path = "/ticket/stats/{mentorId}")
    public String getTicketStatsIndividual(ModelMap model, @PathVariable("mentorId") String mentorId) {
        model.addAttribute("stats", adminTicketService.getTicketStatsDetails(UUID.fromString(mentorId)));
        model.addAttribute("courses", adminCourseService.getSupportedCourseDetails());
        return "role/admin/ticket/overview.html";
    }
